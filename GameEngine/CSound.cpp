...
{
	if (nullptr == GETINSTANCE(CSoundMgr)->GetSoundDevice())
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(GETINSTANCE(CSoundMgr)->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
{
	GETINSTANCE(CSoundMgr)->RegisterToBGM(this);
